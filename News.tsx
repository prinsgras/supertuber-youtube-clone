...
export function News() {
  const { data: videos = [], isLoading } = useQuery<VideoWithChannel[]>({
    queryKey: ["/api/videos", { category: "News" }],
    queryFn: async () => {
      const response = await fetch("/api/videos?category=News");
    ...
    <div className="p-6">
      ...
      {videos.length === 0 ? (
        <div className="text-center py-12">
          <Newspaper className="w-16 h-16 text-gray-300 dark:text-gray-700 mx-auto mb-4" />
